...
while (cont < ARGV.length)
  ...
  File.open(ARGV[cont], "r").each do |line|
    ...
    file_w.write("#{data["unplug_hourTime"]["$date"]} #{data["user_day_code idunplug_station"]} #{data["idplug_station"]} #{data["idunplug_base"]} #{data["idplug_base"]} #{data["user_type"]} #{data["travel_time"]} #{data["ageRange"]} #{data["zip_code"]}\n")
    puts "#{data["unplug_hourTime"]["$date"]} #{data["user_day_code idunplug_station"]} #{data["idplug_station"]} #{data["idunplug_base"]} #{data["idplug_base"]} #{data["user_type"]} #{data["travel_time"]} #{data["ageRange"]} #{data["zip_code"]}"
  end
